import React from "react";
import {
  Box,
  Typography,
  Button,
  Paper,
  Stack,
  Grid,
  Chip,
  IconButton,
  Select,
  MenuItem,
  FormControl,
} from "@mui/material";

import CalendarMonthOutlinedIcon from "@mui/icons-material/CalendarMonthOutlined";
import ArrowOutwardIcon from "@mui/icons-material/ArrowOutward";
import RefreshIcon from "@mui/icons-material/Refresh";
import FileDownloadOutlinedIcon from "@mui/icons-material/FileDownloadOutlined";
import CloseIcon from "@mui/icons-material/Close";

const features = [
  "AI Qualifies Leads",
  "Books Calendar Slots",
  "Meeting Confirmation",
  "WhatsApp Reminder",
  "Email Reminder",
  "24×7 Availability",
];

const bookingGroups = [
  {
    date: "Wed, 22 Jul",
    count: "1 booking",
    bookings: [
      {
        time: "10:00 AM",
        name: "Sami",
        phone: "+91 9099433467",
        project: "Saur Developer, Guj",
        booked: "Booked 21 Jul, 11:43 am",
      },
    ],
  },
  {
    date: "Fri, 24 Jul",
    count: "3 bookings",
    bookings: [
      {
        time: "10:00 AM",
        name: "Harsh",
        phone: "+91 9537010060",
        project: "Rapid Sales 1.0",
        booked: "Booked 23 Jul, 4:40 pm",
      },
      {
        time: "1:00 AM",
        name: "Bhoomi",
        phone: "+91 9131270607",
        project: "Rapid Sales 1.0",
        booked: "Booked 23 Jul, 4:58 pm",
      },
      {
        time: "4:00 PM",
        name: "Harsh",
        phone: "+91 9537010060",
        project: "Rapid Sales 1.0",
        booked: "Booked 23 Jul, 3:18 pm",
      },
    ],
  },
  {
    date: "Sat, 25 Jul",
    count: "5 bookings",
    bookings: [
      {
        time: "11:00 AM",
        name: "Bhoomi",
        phone: "+91 9131270607",
        project: "Rapid Sales 1.0",
        booked: "Booked 23 Jul, 5:54 pm",
      },
      {
        time: "12:00 PM",
        name: "Harsh",
        phone: "+91 9537010060",
        project: "Saur Developer",
        booked: "Booked 23 Jul, 1:22 pm",
      },
    ],
  },
];

const AppointmentBooking = () => {
  return (
    <Box
      sx={{
        minHeight: "100vh",
        width: "100%",
        overflow: "hidden",
        position: "relative",
        bgcolor: "#f8f8f8",
        background:
          "radial-gradient(circle at 65% 38%, rgba(234, 137, 42, 0.13), transparent 27%), radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.9), transparent 35%), #f8f8f8",
        py: { xs: 5, md: 6 },
      }}
    >
      <Box
        sx={{
          maxWidth: 1380,
          mx: "auto",
          px: { xs: 2.5, sm: 4, md: 6 },
        }}
      >
        {/* 
          IMPORTANT:
          alignItems="center" means the Calendar column
          is vertically centered relative to the LEFT content.
        */}
        <Grid container spacing={{ xs: 6, md: 4 }} alignItems="center">
          {/* ================= LEFT SECTION ================= */}
          <Grid item xs={12} md={6}>
            <Box sx={{ maxWidth: 670 }}>
              {/* Badge */}
              <Chip
                icon={
                  <CalendarMonthOutlinedIcon
                    sx={{
                      fontSize: 19,
                      color: "#ef5a16 !important",
                    }}
                  />
                }
                label="AI Appointment Booking"
                sx={{
                  height: 42,
                  px: 1,
                  mb: 3,
                  borderRadius: "30px",
                  bgcolor: "rgba(245, 225, 211, 0.8)",
                  color: "#c75c20",
                  fontSize: 16,
                  fontWeight: 500,
                  letterSpacing: "0.2px",
                }}
              />

              {/* Heading */}
              <Typography
                component="h1"
                sx={{
                  fontSize: {
                    xs: "20px",
                    sm: "30px",
                    lg: "35px",
                  },
                  lineHeight: 1.2,
                  fontWeight: 500,
                  color: "#151b28",
                  letterSpacing: "0.3px",
                  maxWidth: 690,
                  mb: 2.5,
                }}
              >
                Let AI Handle Your{" "}
                <Box
                  component="span"
                  sx={{
                    color: "#4F46E5",
                  }}
                >
                  Meeting
                  <br />
                  Bookings
                </Box>
              </Typography>

              {/* Description */}
              <Typography
                sx={{
                  maxWidth: 710,
                  color: "#485568",
                  fontSize: {
                    xs: "15px",
                    md: "18px",
                  },
                  lineHeight: 1.55,
                  letterSpacing: "0.5px",
                  mb: 4,
                }}
              >
                Your AI Voice Agent automatically qualifies prospects, answers
                common questions, schedules meetings, and sends confirmations —
                ensuring every lead gets an instant response.
              </Typography>

              {/* Feature Cards */}
              <Box
                sx={{
                  display: "grid",
                  gridTemplateColumns: {
                    xs: "1fr",
                    md: "repeat(2, 1fr)",
                  },
                  gap: {
                    xs: 2,
                    md: "20px 22px",
                  },
                  width: "100%",
                  maxWidth: "720px",
                }}
              >
                {features.map((feature) => (
                  <Paper
                    key={feature}
                    elevation={0}
                    sx={{
                      height: {
                        xs: 40,
                        md: 60,
                      },
                      width: "100%",
                      display: "flex",
                      alignItems: "center",
                      gap: 2,
                      px: {
                        xs: 2,
                        md: 2.5,
                      },
                      borderRadius: "16px",
                      bgcolor: "rgba(255,255,255,0.55)",
                      border: "1px solid #d6dbe1",
                      boxShadow: "0 3px 7px rgba(20, 30, 50, 0.08)",
                      boxSizing: "border-box",
                    }}
                  >
                    {/* Check Circle */}
                    <Box
                      sx={{
                        width: {
                          xs: 30,
                          md: 35,
                        },
                        height: {
                          xs: 30,
                          md: 35,
                        },
                        minWidth: {
                          xs: 30,
                          md: 35,
                        },
                        borderRadius: "50%",
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        bgcolor: "#f6eee9",
                      }}
                    >
                      <Typography
                        component="span"
                        sx={{
                          color: "#e87532",
                          fontSize: 20,
                          fontWeight: 600,
                          lineHeight: 1,
                          fontFamily: "Arial, sans-serif",
                        }}
                      >
                        ✓
                      </Typography>
                    </Box>

                    <Typography
                      sx={{
                        fontSize: {
                          xs: 14,
                          md: 16,
                        },
                        color: "#17202d",
                        fontWeight: 500,
                        whiteSpace: "nowrap",
                      }}
                    >
                      {feature}
                    </Typography>
                  </Paper>
                ))}
              </Box>

              {/* Button */}
              <Button
                variant="contained"
                endIcon={<ArrowOutwardIcon />}
                sx={{
                  mt: 3.5,
                  width: 218,
                  height: 56,
                  borderRadius: "10px",
                  textTransform: "none",
                  bgcolor: "#f85b08",
                  fontSize: 18,
                  fontWeight: 700,
                  boxShadow: "none",
                  "&:hover": {
                    bgcolor: "#df4f03",
                    boxShadow: "none",
                  },
                }}
              >
                Book Demo
              </Button>
            </Box>
          </Grid>

          {/* ================= RIGHT SECTION ================= */}
          <Grid
            item
            xs={12}
            md={6}
            sx={{
              display: "flex",
              alignItems: "center",
            }}
          >
            <Box
              sx={{
                width: "100%",
                display: "flex",
                justifyContent: {
                  xs: "center",
                  md: "flex-end",
                },
              }}
            >
              <Paper
                elevation={0}
                sx={{
                  width: "100%",
                  maxWidth: 680,
                  height: { xs: 470, lg: 400 },
                  overflow: "hidden",
                  borderRadius: "20px",
                  bgcolor: "rgba(255,255,255,0.68)",
                  border: "1px solid #d6dce5",
                  boxShadow: "0 24px 35px rgba(50, 38, 20, 0.2)",
                }}
              >
                {/* Calendar Header */}
                <Box
                  sx={{
                    px: 2.2,
                    pt: 1.5,
                    pb: 1.3,
                    borderBottom: "1px solid #e4e7eb",
                  }}
                >
                  <Stack
                    direction="row"
                    justifyContent="space-between"
                    alignItems="center"
                  >
                    <Box>
                      <Stack direction="row" spacing={0.7} alignItems="center">
                        <CalendarMonthOutlinedIcon
                          sx={{
                            color: "#e87838",
                            fontSize: 19,
                          }}
                        />

                        <Typography
                          sx={{
                            fontSize: 16,
                            fontWeight: 700,
                            color: "#27313d",
                          }}
                        >
                          Booking Calendar
                        </Typography>
                      </Stack>

                      <Typography
                        sx={{
                          fontSize: 9,
                          color: "#5e6977",
                          mt: 0.3,
                        }}
                      >
                        Bookings your agent makes on calls, day by day
                      </Typography>
                    </Box>

                    <Button
                      size="small"
                      startIcon={<RefreshIcon sx={{ fontSize: 14 }} />}
                      sx={{
                        minWidth: 65,
                        height: 28,
                        borderRadius: "6px",
                        textTransform: "none",
                        fontSize: 9,
                        color: "#45505d",
                        bgcolor: "#fff",
                        border: "1px solid #e2e5e8",
                      }}
                    >
                      Refresh
                    </Button>
                  </Stack>
                </Box>

                {/* Filters */}
                <Box
                  sx={{
                    px: 1.7,
                    py: 1,
                    display: "flex",
                    alignItems: "center",
                    gap: 1,
                    borderBottom: "1px solid #e8eaed",
                  }}
                >
                  <Paper
                    variant="outlined"
                    sx={{
                      height: 34,
                      width: 102,
                      px: 1,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "space-between",
                      borderRadius: "7px",
                      borderColor: "#d8dde3",
                    }}
                  >
                    <Typography sx={{ fontSize: 9, color: "#596574" }}>
                      01-07-2026
                    </Typography>

                    <CalendarMonthOutlinedIcon
                      sx={{ fontSize: 12, color: "#52606d" }}
                    />
                  </Paper>

                  <Typography sx={{ color: "#a0a7b0" }}>→</Typography>

                  <Paper
                    variant="outlined"
                    sx={{
                      height: 34,
                      width: 102,
                      px: 1,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "space-between",
                      borderRadius: "7px",
                      borderColor: "#d8dde3",
                    }}
                  >
                    <Typography sx={{ fontSize: 9, color: "#596574" }}>
                      07-08-2026
                    </Typography>

                    <CalendarMonthOutlinedIcon
                      sx={{ fontSize: 12, color: "#52606d" }}
                    />
                  </Paper>

                  <FormControl size="small">
                    <Select
                      defaultValue="Confirmed"
                      sx={{
                        height: 34,
                        minWidth: 82,
                        fontSize: 9,
                        borderRadius: "7px",
                        "& .MuiSelect-select": {
                          py: 0.8,
                        },
                      }}
                    >
                      <MenuItem value="Confirmed">Confirmed</MenuItem>
                      <MenuItem value="Pending">Pending</MenuItem>
                    </Select>
                  </FormControl>

                  <Box sx={{ flexGrow: 1 }} />

                  <Typography
                    sx={{
                      fontSize: 8,
                      color: "#64707d",
                      whiteSpace: "nowrap",
                    }}
                  >
                    13 bookings
                  </Typography>

                  <Button
                    startIcon={
                      <FileDownloadOutlinedIcon sx={{ fontSize: 12 }} />
                    }
                    sx={{
                      minWidth: 76,
                      height: 28,
                      fontSize: 8,
                      textTransform: "none",
                      color: "#5c6875",
                      border: "1px solid #e1e4e8",
                      borderRadius: "6px",
                    }}
                  >
                    Export Excel
                  </Button>
                </Box>

                {/* Calendar Bookings */}
                <Box sx={{ height: 300, overflow: "hidden" }}>
                  {bookingGroups.map((group) => (
                    <Box key={group.date}>
                      {/* Date Header */}
                      <Box
                        sx={{
                          height: 31,
                          px: 2,
                          display: "flex",
                          alignItems: "center",
                          bgcolor: "#f4f5f6",
                          borderBottom: "1px solid #e6e8eb",
                        }}
                      >
                        <CalendarMonthOutlinedIcon
                          sx={{
                            fontSize: 10,
                            color: "#8a94a0",
                            mr: 0.8,
                          }}
                        />

                        <Typography
                          sx={{
                            fontSize: 8,
                            fontWeight: 700,
                            color: "#52606d",
                          }}
                        >
                          {group.date}
                        </Typography>

                        <Typography
                          sx={{
                            fontSize: 7,
                            color: "#8a949f",
                            ml: 0.6,
                          }}
                        >
                          - {group.count}
                        </Typography>
                      </Box>

                      {/* Booking Rows */}
                      {group.bookings.map((booking, index) => (
                        <Box
                          key={`${group.date}-${index}`}
                          sx={{
                            minHeight: 43,
                            px: 2,
                            display: "flex",
                            alignItems: "center",
                            borderBottom: "1px solid #eceef0",
                          }}
                        >
                          <Box sx={{ width: "16%" }}>
                            <Typography
                              sx={{
                                fontSize: 10,
                                fontWeight: 700,
                                color: "#384552",
                              }}
                            >
                              {booking.time}
                            </Typography>
                          </Box>

                          <Box sx={{ flexGrow: 1 }}>
                            <Typography
                              sx={{
                                fontSize: 10,
                                fontWeight: 700,
                                color: "#46515e",
                              }}
                            >
                              {booking.name}
                            </Typography>

                            <Stack direction="row" spacing={1} sx={{ mt: 0.2 }}>
                              <Typography
                                sx={{
                                  fontSize: 7,
                                  color: "#88919c",
                                }}
                              >
                                ☎ {booking.phone}
                              </Typography>

                              <Typography
                                sx={{
                                  fontSize: 7,
                                  color: "#88919c",
                                }}
                              >
                                {booking.project}
                              </Typography>

                              <Typography
                                sx={{
                                  fontSize: 7,
                                  color: "#88919c",
                                }}
                              >
                                {booking.booked}
                              </Typography>
                            </Stack>
                          </Box>

                          <Chip
                            label="Confirmed"
                            size="small"
                            sx={{
                              height: 20,
                              fontSize: 7,
                              bgcolor: "#eef8f1",
                              color: "#27815b",
                              fontWeight: 600,
                              mr: 1.5,
                            }}
                          />

                          <IconButton
                            size="small"
                            sx={{
                              width: 22,
                              height: 22,
                              color: "#8c98a4",
                            }}
                          >
                            <CloseIcon sx={{ fontSize: 12 }} />
                          </IconButton>
                        </Box>
                      ))}
                    </Box>
                  ))}
                </Box>
              </Paper>
            </Box>
          </Grid>
        </Grid>
      </Box>
    </Box>
  );
};

export default AppointmentBooking;
